# Sprint 1 Planning Session
Date: [2026/01/14]
Sprint Goal: 
## Attendees
- Product Owner: Fancy van Zyle
- Scrum Master: Boss de Klerk
- Development Team: Ricky, Rick, John, Jane and Jennifer
## Velocity Target
- 5 story points completion every week
## Selected User Stories
| Story # | Title | Story Points |
| Story #1 | As an employee I want to be able to book a conference room | 3 |
| Story #2 | As an employee I want to view recurring meetings | 3 |
| Story #3 | As an employee I want to filter the rooms by capacity | 1 |
| Story #4 | As an employee I want to cancel a booking | 3 |
| Story #5 | As an employee I want to be able to view the room equipment requirements | 1 |
| Story #6 | As an Admin I want to log into the administartion dashboard | 2 |
| Story #10 | As an admin I want to be able to generate a usage report | 3 |
## Dependencies
- Story 2, 3 and 4 are dependent on story 1
- Story 10 is dependant on story 6
## Risks
| Risk | Probability | Impact | Mitigation |
## Standup Cadence
Time: 08:30 a.m.
Format: Yesterday / Today / Blockers